...
public abstract class SuperPower : ScriptableObject
{
    ...
    public virtual void Init(GameObject player, GameObject playerRoot, float levelRatio)
    {
        ...
        _playerHealth = player.GetComponent<IDamageable>(); // абстракный класс для различных суперсил, возможно тут стоило бы использовать паттер декоратор, но додумался я об этом достаточно поздно
       // _levelRatio = levelRatio;

       HPBar.SMBDead += GameOver;
        
        _currentParticles = Instantiate(_particleSystem, _particleSystemPos, EnemyBoxer.transform.rotation, EnemyBoxer.transform); // создаем основной gameobject с партиклами
        
        _myAnimator.Play(_curAnim.name, 0); // запускаем основную анимацию суперсилы
    }

    protected virtual void GameOver(string tag)
    {
        _myAnimator.SetLayerWeight(1, 1);
        EnemyBoxer.CurrentSuperPower = null;  // тут не до конца наладил систему смерти и завершения игры, так как понял, что это не требуется?
    ...
    public void Charging(float speed)  // тут мы меняем скорость анимации, чтобы сделать эффект "зарядки"
    {
    ...
    {
        SuperAttackOver?.Invoke();    //отправляем состоянию, что суперсила завершилась
    }

    public virtual void Hitting()
    {
        _currentParticles.GetComponentInChildren<ParticleSystem>().Play(); // тут мы включаем партиклы, которые остались
    }

    public virtual void SetForceToPlayer(Rigidbody playerRB,float force, Vector3 centrePos, float distance)
    {
        PlayerRBOn.Invoke(); // включаем рэг долл
        playerRB.AddExplosionForce(force, centrePos, distance); // придаем основной кости рэгдолла силу
    }
}
